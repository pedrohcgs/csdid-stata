...
csdid y x, ivar(id) time(year) gvar(group) method(reg) analytical
...
capture noisily csdid y x, ivar(id) time(year) gvar(group) compute_inffunc(false) analytical
local rc = _rc
...
foreach method in dr reg ipw {
    import delimited using "`root'/tests/fixtures/parity/rt008/inputs/input.csv", clear asdouble
    quietly csdid y x, ivar(id) time(year) gvar(group) method(`method') analytical
    tempname IF UG
    matrix `IF' = e(inffunc)
