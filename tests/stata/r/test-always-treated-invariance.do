...
program define rt004_save_att
    ...
    quietly csdid y, ivar(uid) time(t) gvar(g) method(`method') `notyet' analytical ///
        anticipation(`anticipation') `fast' bal(none) storeall
    matrix ATT = e(attgt)
    clear
...
quietly csdid y, ivar(uid) time(t) gvar(g) method(dr) notyet analytical base_period(varying) bal(none) storeall
matrix ATT = e(attgt)
...
foreach method in dr reg ipw {
    ...
    quietly csdid y, ivar(uid) time(t) gvar(g) method(`method') analytical nevertreated base_period(varying) bal(none) storeall
...
quietly csdid y, ivar(uid) time(t) gvar(g) method(dr) analytical nevertreated base_period(varying) bal(none) storeall
