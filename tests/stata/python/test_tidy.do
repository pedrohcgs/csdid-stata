...
local mp_nobs = r(N)
drop tag_id
csdid lemp, ivar(countyreal) time(year) gvar(first_treat) method(reg) analytical nevertreated base_period(varying) bal(none) storeall
...
import delimited using "`root'/tests/fixtures/parity/py022/inputs/mpdta.csv", clear asdouble
quietly csdid lemp, ivar(countyreal) time(year) gvar(first_treat) method(reg) analytical nevertreated base_period(varying) bal(none) storeall
