...
script_path <- if (length(file_arg)) sub("^--file=", "", file_arg[[1]]) else "tools/parity/generators/f003/generate.R"
...
fixture <- file.path(root, "tests/fixtures/parity/f003")
...
agg <- suppressWarnings(aggte(mp, type = "simple", bstrap = FALSE, cband = FALSE))
...
manifest <- list(
  ...
  generators = list(list(runtime = "R", command = "Rscript tools/parity/generators/f003/generate.R", path = "tools/parity/generators/f003/generate.R")),
  ...
  rng = NULL,
  expected_outputs = list(list(path = "expected/r/aggte.csv", schema = "aggte-simple")),
  comparison_plan = list(list(actual = "expected/new-stata/aggte.csv", expected = "expected/r/aggte.csv", tolerance_id = "TOL001", key_columns = c("type"))),
