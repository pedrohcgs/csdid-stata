...
script_path <- if (length(file_arg)) sub("^--file=", "", file_arg[[1]]) else "tools/parity/generators/f005/generate.R"
...
fixture <- file.path(root, "tests/fixtures/parity/f005")
...
agg <- suppressWarnings(aggte(mp, type = "calendar", bstrap = FALSE, cband = FALSE))
...
manifest <- list(
  ...
  generators = list(list(runtime = "R", command = "Rscript tools/parity/generators/f005/generate.R", path = "tools/parity/generators/f005/generate.R")),
  ...
  rng = NULL,
  expected_outputs = list(list(path = "expected/r/aggte.csv", schema = "aggte-calendar")),
