...
quietly csdid lemp, time(year) gvar(firsttreat) method(dr) analytical
...
quietly csdid lemp, time(year) gvar(firsttreat) method(dr) analytical
matrix DRR = e(attgt)
quietly csdid lemp, time(year) gvar(firsttreat) method(ipw) analytical
matrix IPR = e(attgt)
...
import delimited using "`root'/tests/fixtures/parity/py019/inputs/mpdta.csv", clear asdouble
quietly csdid lemp, ivar(countyreal) time(year) gvar(firsttreat) method(reg) base_period(varying) analytical
matrix BV = e(attgt)
...
quietly csdid lemp, ivar(countyreal) time(year) gvar(firsttreat) method(reg) analytical
matrix FP = e(attgt)
...
capture csdid lemp, ivar(countyreal) time(yearstr) gvar(firsttreat) method(dr) analytical
assert _rc == 198
